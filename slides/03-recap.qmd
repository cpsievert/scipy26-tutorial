---
title: "Recap"
subtitle: "Foundations + chatlas basics"
format:
  revealjs:
    theme: [default, theme.scss]
    slide-number: c/t
    width: 1280
    height: 720
    incremental: false
    transition: fade
    code-line-numbers: false
---

## Where we've been {.center}

::: fragment
🧭 **Foundations**: how to think about LLMs (strengths and limitations)
:::

::: fragment
🐍 **chatlas**: how to talk to LLMs from Python
:::

## The big idea, again {.center}

::: fragment
LLMs are **jagged** — great at some things, bad at others (e.g. counting).
:::

::: fragment
Correctness can improve with the right **context and constraints**.
:::

::: fragment
LLMs are _really good_ at writing code — so let them, and verify the result.
:::

::: fragment
**Agent = LLM + tools + loop.**
:::

## chatlas, so far {.center}

::: fragment
One interface (`Chat{Provider}`) to many LLMs — swap providers, keep your code.
:::

::: fragment
`.chat()` echoes responses; `.stream()` yields the chunks.
:::

::: fragment
Chat objects are **stateful** — they remember the conversation (`get_turns()`, `set_turns()`).
:::

::: fragment
A **system prompt** shapes every response, invisibly, without RAG or tool calling.
:::

## Quick check ✅ {.center}

::: fragment
What's the difference between a system and user prompt?
:::

::: fragment
What method yields a chatbot in 1 line of code?
:::

::: fragment
Why might you want to use `.stream()` over `.chat()`?
:::

## Up next {.center background-color="#0F3B5D"}

::: fragment
🔧 **Tool calling** — give the LLM real capabilities
:::

::: fragment
📦 **Structured data extraction**
:::

::: fragment
💬 **shinychat** — turn a chat into an app
:::
