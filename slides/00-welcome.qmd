---
title: "Safe, Verifiable, & Maintainable AI Apps in Python"
subtitle: "Welcome & Setup"
author: "Carson Sievert · Posit"
format:
  revealjs:
    theme: [default, theme.scss]
    slide-number: c/t
    width: 1280
    height: 720
    incremental: false
    transition: fade
    code-line-numbers: false
callout-appearance: simple
---

## Hi, I'm Carson 👋 {.center}

::: {.columns}
::: {.column width="45%"}
::: {.placeholder}
![](images/me-square.png)
:::
:::
::: {.column width="55%" .center}

- Open source software engineer at **Posit**
- Work on **chatlas**, **shinychat**, **querychat**
- Background in **statistics** and **data science**

:::
:::


## Posit’s mission... {.center}


> ### To create open-source software for data science, scientific research, and technical communication.

::: {.r-hstack .fragment}
![](images/shiny.svg){height="200px"}
![](images/quarto.svg){height="200px"}
![](images/great_tables.svg){height="200px"}
:::

::: {.r-hstack .fragment style="margin-top: -2em; margin-left: 4em;"}
![](images/chatlas.png){height="200px"}
![](images/shinychat.svg){height="200px"}
![](images/querychat.png){height="200px"}
:::




## Visit our Booth! {.center}

Many notable names in open source:

- Hadley Wickham (R, tidyverse, ggplot2, etc)
- Joe Cheng (Posit CTO, Shiny)
- Winston Chang (Posit Assistant, Shiny)
- Josh Taillon (Shiny)
- Carlos Scheidegger (Quarto)
- and others



## Who's in the room? {.center}

Quick show of hands 🙋

::: columns
::: {.column width="50%"}

::: fragment
- Use **Python** on daily basis?
:::
::: fragment
- Use **Copilot** for AI assistance?
:::
::: fragment
- Use **Claude Code** or **Codex** for AI assistance?
:::

:::

::: {.column width="50%"}

::: fragment
- Called an **LLM API** in code before?
:::
::: fragment
- Built or shipped an **LLM app**?
:::
::: fragment
- Would like to ship an **AI app** that lets others explore your data more easily?
:::
:::
:::


##

| Section | Time |
|---|---|
| **Welcome & Setup** | 30 min |
| **LLM foundations** | 30 min |
| **Chat basics** | 30 min |
| Break+Recap | 10 min |
| **Tool calling / agents** | 25 min |
| **Structured data extraction** | 15 min |
| **Chatbots** | 20 min |
| Break+Recap | 10 min |
| **Production tooling** | 25 min |
| **Evals** | 20 min |
| **querychat** — self-service analytics case study | 15 min |

: {tbl-colwidths="[75,25]"}




## Website (materials) 📁

::: {.hcenter .center}

![](images/website-qr.svg){height="350px"}

### [bit.ly/scipy26](https://bit.ly/scipy26){.big}

The website has links to these slides and other helpful things. 

Please bookmark it for easy access during the workshop.

:::



## LLM access 🤖 {.center}

Exercises require access to an LLM. 

::: fragment
### Things will just work if you use the provided environment
:::

::: fragment
::: {.callout-tip}
If you'd rather work locally, follow the instructions in [the website](https://bit.ly/scipy26).
:::
:::


## Computing environment 💻 {.center}

1. Go to [dev.workshop.posit.team](https://dev.workshop.posit.team)
2. Sign in via your GitHub account (or create a new account)
3. Click **New session** at the top
4. Under **Environment**, select the appropriate image:
    * `654654567442.dkr.ecr.us-east-2.amazonaws.com/ptd-adhoc-pwb:session-jammy-scipy-2026.05.0-218.pro1-76c65f7f`
5. Click **Launch**.
    * Wait for longer than you expect -- blank screen is normal while the environment is being provisioned.
7. Click **Open** -> **Open Folder** -> Enter `/opt/scipy26-tutorial`

::: {.callout-tip}
If you'd rather work locally, follow the instructions in [the website](https://bit.ly/scipy26).
:::


## Exercise: Test run {.center background-color="#0e3b5c"}

1. Open the `exercises/00-chat-hello.py` file.
2. Run it in the provided environment.
    * Press play button or Cmd/Ctrl+Enter
3. You should see a joke response from the LLM.
4. If you prefer a notebook
    * Open command palette with Cmd/Ctrl+Shift+Enter
    * Enter "New Jupyter notebook"
    * Copy/paste over code over


<br>

{{<countdown "3:00" >}}


## Ground rules {.center}

- ❓ **Interrupt with questions** — this is a tutorial, not a keynote.
- 🤝 **Pair up** with a neighbor — it's more fun and faster.
- 🛑 **Stuck? Flag it.** Don't suffer silently during exercises.


# It's ok to feel conflicted about AI, I certainly am! {.center}